...
function Navbar() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        setSubLinks(Array.isArray(categoriesData) ? categoriesData : [])
      } catch (error) {
  ...
  useEffect(() => {
    ...
    setHideNavbar(
      hideOnPaths.some((path) => matchPath({ path }, location.pathname))
    )
  }, [location])
  ...
  return (
    <>
      {!hideNavbar && (
        <div
          className={`fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-center border-b-[1px] border-b-richblack-700 ${
        ...
        >
                          ...
                          {loading ? (
                            ...
                            subLinks.map((subLink, i) => (
                              <Link key={i} to={`/catalog/${subLink.name}`}>
                                <p className="mx-auto rounded p-3 hover:bg-richblack-200">
                                  {subLink.name}
                                </p>
                              </Link>
                            ))
          ...
          {/* Mobile Menu */}
          {isMenuOpen && (
            <div className="fixed left-0 top-14 flex w-full flex-col items-center gap-y-4 bg-richblack-800 p-4 md:hidden">
              <nav>
                          ...
                          {isCatalogOpen && (
                            <div className="flex w-full flex-col items-center gap-y-2">
                              {loading ? (
                                ...
                                subLinks.map((subLink, i) => (
                                  <Link key={i} to={`/catalog/${subLink.name}`}>
                                    <p className="w-full rounded p-2 text-center hover:bg-richblack-200">
                                      {subLink.name}
                                    </p>
                                  </Link>
                                ))
